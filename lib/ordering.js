// lib/ordering.js
//
// Extracted from timhortons-chat.js so orders.js can also finalize an order
...
export async function confirmOrder({ items: rawItems, note }, tenantId, customerId, phoneVerified, existingOrderId, sessionId, staffSupervised = false) {
    ...
    if (existingOrderId) {
        // Scoped to this tenant AND this customer — without both filters, any
        // caller that can guess/obtain an order UUID (e.g. via a spoofed
        // customerId on the chat endpoint) could overwrite or cancel another
        // customer's order. Every other query in this file already scopes by
        // tenant_id; this lookup was the one exception.
    ...
    // Staff-finalize specific: the AI sometimes "re-confirms" an order that
    // already exists without passing its order_id (e.g. after a takeover
    // conversation that didn't actually change anything). Rather than create
    // an exact duplicate, check the customer's open orders for one with
    // identical items and treat this as a no-op against that order instead.
    if (staffSupervised && customerId) {
        ...
        const sameItems = (a, b) => {
            ...
            const key = (i) => `${i.menu_item_id}:${i.qty}`;
            const setA = new Set(a.map(key));
            return b.every((i) => setA.has(key(i))) && a.every((i) => b.some((j) => key(j) === key(i)));
        };

        const duplicate = (openOrders || []).find((o) => sameItems(o.items, fullItems));
        if (duplicate) {
            return {
                ...
                already_matches: true,
                note_for_ai: `Order #${duplicate.order_number} already has exactly this — nothing needs to change. Don't create a new order or mention any duplicate; just confirm the existing order's total to whoever you're talking to.`
            };
        }
    }
