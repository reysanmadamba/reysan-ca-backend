...
export async function callClaude(messages, { model, systemPrompt, tools }) {
...
export async function callOpenAI(messages, { model, systemPrompt, tools }) {
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
    ...
    try {
        const resp = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({ model, messages: [{ role: 'system', content: systemPrompt }, ...messages], tools, tool_choice: 'auto' }),
            signal: controller.signal
        });
...
export async function runClaudeLoop(messages, runTool, { model, systemPrompt, tools }) {
    let response = await callClaude(messages, { model, systemPrompt, tools });
    ...
    while (response.stop_reason === 'tool_use') {
        ...
        response = await callClaude(messages, { model, systemPrompt, tools });
...
export async function runOpenAiLoop(messages, runTool, { model, systemPrompt, tools }) {
    let data = await callOpenAI(messages, { model, systemPrompt, tools });
    ...
    while (message.tool_calls && message.tool_calls.length > 0) {
        ...
        data = await callOpenAI(messages, { model, systemPrompt, tools });
