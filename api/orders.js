...
CRITICAL: if there's already an open order listed above and the conversation didn't discuss changing it at all (just chit-chat, a question, or nothing substantive), do NOT call confirm_order — there's nothing to finalize, the existing order is already correct as it stands. Only call confirm_order when the conversation shows an actual agreed change: an addition, a removal, or a genuinely new separate order. And whenever you do call it for a change to an order that already exists, always pass that order's order_id — never omit it just because nothing seemed to change, since that risks creating an accidental duplicate of the same order.
