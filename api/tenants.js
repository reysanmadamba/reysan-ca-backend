...
async function getTenantDetail(tenantId) {
  ...
  // AI usage/cost — bounded to the last 30 days of chat_logs so this stays
  // fast even as history grows. Aggregated here in JS rather than a DB
  // function, since volume at this scale doesn't need it yet.
  ...
  // Bucket the last 14 days for the chart, oldest first.
  const dailyMap = new Map();
  for (let i = 13; i >= 0; i--) {
    const d = new Date(now.getTime() - i * 24 * 60 * 60 * 1000);
    dailyMap.set(d.toISOString().slice(0, 10), { date: d.toISOString().slice(0, 10), cost: 0, messages: 0 });
  }
  rows.forEach((r) => {
